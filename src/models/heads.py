...
def build_autoencoder(cfg: Config) -> nn.Module:
    ...
    The training window is the pooled/resampled length (common_window_samples),
    not the native signal window, since every animal is resampled to it.
    """
    window = cfg.data.common_window_samples
    m = cfg.model
